...
import { faGithub, faInstagram, faLinkedinIn, faReddit, faRedditAlien, faTelegram, faTelegramPlane, faTiktok } from "@fortawesome/free-brands-svg-icons";
                    ...
                    socials.map((content, index) => {
                        return content.link != "" ?
                            <Link href={content.link} key={index} target="_blank">
                                <FontAwesomeIcon icon={content.icon} className="text-zinc-500 hover:text-white hover:scale-125 mx-3" width={23} height={23} />
                            </Link> : <> </>
                    })
                }
            </div>
